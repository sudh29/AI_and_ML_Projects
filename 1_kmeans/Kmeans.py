...
def cluster_customers(data_train, data_test):
    ...
    km_4 = KMeans(
        n_clusters=4, init="k-means++", n_init=10, max_iter=50, tol=0.05, random_state=0
    )
    ...
    silhouette_score_opt = metrics.silhouette_score(
        sd_train, labels, metric="euclidean", sample_size=1000, random_state=0
    )
    # print(silhouette_score_opt)

    # Silhouette score for kmeans_4 and sd_train
    labels = kmeans_4.labels_
    silhouette_score_4 = metrics.silhouette_score(
        sd_train, labels, metric="euclidean", sample_size=1000, random_state=0
    )
    ...
    silhouette_score_4_test = metrics.silhouette_score(
        sd_test, labels, metric="euclidean", sample_size=1000, random_state=0
    )
    ...
    cluster_centers = pd.DataFrame(
        test_kmeans.cluster_centers_, columns=df_train.columns
    ).to_numpy()
    ...
    for i in range(6):
        ...
        e_distance = np.sqrt(
            np.sum(np.square(np.subtract(cluster_centers[i], first_ob)))
        )
